...
- **mDNS 发现**: `http://<device_id>.local:8000`（API 启动时通过 zeroconf 注册 `_http._tcp` 服务）
- **API 版本**: v1.1.0
- **Content-Type**: `application/json`
- **认证**: JWT Bearer Token（通过 `LELAMP_JWT_SECRET` 签名）
...
获取设备 ID、主机名、型号、版本和绑定密钥（用于设备绑定流程）。
...
**说明**:
- `device_secret` 在设备首次 WiFi 配网时自动生成（`secrets.token_hex(8)`，16 字符）
- 存储在 `/var/lib/lelamp/setup_status.json`
- 前端通过此端点获取设备信息，展示可复制的 secret 和一键绑定按钮
...
替代旧的 CLI 脚本 `scripts/tools/generate_client_token.py`。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `room` | string | 是 | 房间名（1-128 字符） |
| `identity` | string | 否 | 用户身份（默认使用 JWT 认证身份） |
...
| `LELAMP_DEVICE_SECRET` | - | 设备绑定密钥（fallback，优先使用 setup_status.json 中的自动生成值） |
...
### v1.1.0 (2026-04-01)
- 新增 `GET /api/system/device` 设备信息端点
- 新增 `POST /api/livekit/token` LiveKit Token API（替代 CLI 脚本）
- JWT 认证加固（`LELAMP_JWT_SECRET` 签名密钥）
- 设备绑定流程（`hmac.compare_digest()` 验证）
- mDNS 设备自动发现（zeroconf `_http._tcp`）
- Vue 前端托管（`LELAMP_WEB_DIST`）
