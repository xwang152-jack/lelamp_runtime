...
LeLamp 采用多服务架构，各服务独立管理：

| 服务 | 文件 | 功能 |
|------|------|------|
| **lelamp-livekit.service** | `scripts/services/lelamp-livekit.service` | LiveKit 语音交互代理 |
| **lelamp-api.service** | 通过 `scripts/setup/setup_api_startup.sh` 生成 | FastAPI REST API 服务 |
| **lelamp-captive-portal.service** | `scripts/services/lelamp-captive-portal.service` | 首次配置 Portal（按需启动） |
...
```bash
# 查看状态
sudo systemctl status lelamp-livekit.service
sudo systemctl status lelamp-api.service

# 重启服务
sudo systemctl restart lelamp-livekit.service
sudo systemctl restart lelamp-api.service

# 查看日志
sudo journalctl -u lelamp-livekit.service -f
sudo journalctl -u lelamp-api.service -f
```

> **详细配置说明请参考**: [AUTO_STARTUP_GUIDE.md](AUTO_STARTUP_GUIDE.md)
