...
> 💡 **快速提示**: 如果您已经购买了预装系统的 LeLamp 设备，可以直接跳到 [阶段二：获取 API 密钥](#阶段二获取-api-密钥) 和 [阶段三：环境变量配置](#阶段三环境变量配置)，设备已预装所有依赖。
...
| [ ] **Node.js** | 18+ (可选，仅前端开发需要) | `node --version` | https://nodejs.org |
...
```bash
# 方式 1: 使用下载脚本（推荐）
bash scripts/tools/download_models.sh
...
详细配置说明请参考：[边缘视觉配置指南](FEATURES.md)
...
# 使用启动脚本（自动处理 GPIO 权限）
sudo bash scripts/services/start_api_server.sh
```
...
# 运行自动启动设置脚本
bash scripts/setup/setup_full_service.sh
```
...
# 运行完整服务系统设置脚本
bash scripts/setup/setup_all_services.sh
```
...
# 安装 Captive Portal 服务
bash scripts/services/install_captive_portal.sh
```
...
# 设置 LiveKit Tmux 服务
bash scripts/setup/setup_livekit_tmux_service.sh
```

**管理服务**：
```bash
# 查看状态
bash scripts/services/livekit_service_manager.sh status

# 查看日志
bash scripts/services/livekit_service_manager.sh logs

# 连接到 tmux 会话
bash scripts/services/livekit_service_manager.sh attach

# 重启服务
bash scripts/services/livekit_service_manager.sh restart
```
...
> **注意**: 前端有自己的 `package.json`，根目录不再有 pnpm workspace。所有前端命令都需要在 `web/` 目录下执行。
...
- [ ] .env 文件已创建
- [ ] LIVEKIT_URL 已配置（可选，仅远程访问需要）
- [ ] LIVEKIT_API_KEY 已配置（可选，仅远程访问需要）
- [ ] LIVEKIT_API_SECRET 已配置（可选，仅远程访问需要）
...
```bash
# API 诊断
uv run python scripts/tools/diagnose_api.py
