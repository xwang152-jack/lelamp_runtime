...
- **环境变量**: `LELAMP_JWT_SECRET`
- **生成方法**: `python -c "import secrets; print(secrets.token_hex(32))"`
- **安全要求**: 生产环境必须设置固定密钥，切勿泄露
- **降级行为**: 未设置时自动生成随机密钥，每次重启后所有已签发 Token 失效，并在启动日志中输出警告
...
设备首次 WiFi 配置成功后自动生成 `device_secret`，用于设备绑定认证：

- **生成**: 首次 WiFi 设置时自动生成 16 位十六进制字符
- **存储**: `/var/lib/lelamp/setup_status.json`
- **回退**: 可通过 `LELAMP_DEVICE_SECRET` 环境变量手动指定
- **获取**: `GET /api/system/device` 返回设备信息 + secret

### 密钥验证安全

- 使用 `hmac.compare_digest()` 进行常量时间比较，防止时序攻击
- 前端设备信息卡片显示可复制的密钥和绑定按钮
- 密钥仅在首次生成时展示，后续通过 API 获取需认证
...
LELAMP_JWT_SECRET=your_jwt_secret_here  # 必需，生成: python -c "import secrets; print(secrets.token_hex(32))"
# LELAMP_DEVICE_SECRET 通常无需手动配置，首次 WiFi 设置时自动生成
```
...
- 无效令牌尝试
- 设备绑定事件
- LiveKit Token 生成（`POST /api/livekit/token`，强制使用已认证用户身份）
- JWT 密钥缺失警告
