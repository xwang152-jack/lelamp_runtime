...
│  │  - /api/system  - /api/livekit/*  - /api/ws/*          │    │
│  └─────────────────────────────────────────────────────────┘    │
│                              ↓                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  服务层 (Business Logic)                                 │    │
│  │  - AuthService  - DeviceService  - SettingsService       │    │
│  │  - MDnsService  - LiveKitTokenService                    │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
### 3. mDNS 设备自动发现 (`lelamp/api/services/mdns_service.py`) 🆕

**设计理念**: 零配置局域网发现、静默降级

API 启动时通过 zeroconf 注册 `_http._tcp` 服务，用户无需知道设备 IP 即可通过 mDNS 名称访问：

```
http://<device_id>.local:8000
```

例如设备 ID 为 `lelamp`，则访问 `http://lelamp.local:8000`。
...
class MDnsService:
    ...
    功能:
    - 注册 _http._tcp 服务到局域网
    - 零配置自动发现（Bonjour/Avahi 兼容）
    - zeroconf 不可用时静默降级，不影响 API 启动
    """
```

**关键特性**:
- **服务类型**: `_http._tcp`（标准 HTTP 服务）
- **端口**: 与 FastAPI 一致（默认 8000）
- **降级策略**: zeroconf 未安装或注册失败时记录警告，不阻塞启动

#### 环境变量

| 环境变量 | 默认值 | 说明 |
|---------|--------|------|
| `LELAMP_WEB_DIST` | `web/dist` | Vue 构建输出路径（用于单端口部署） |
...
用户打开前端 → GET /api/system/device 获取设备信息
   ↓
前端显示设备信息卡片（device_id、device_secret 可复制）
   ↓
用户点击"绑定设备" → POST /api/auth/bind-device
   ↓
服务端 hmac.compare_digest() 验证 device_secret（不存储明文）
   ↓
...
FastAPI 直接托管 Vue 构建输出，实现单端口部署（API + 前端同端口 8000）：

```python
# app.py 中的 StaticFiles 挂载
app.mount("/", StaticFiles(directory=web_dist, html=True), name="web")
```

**关键特性**:
- **SPA 路由**: 非 API/非静态资源路径自动回退到 `index.html`
- **路径排除**: `/api`、`/health`、`/docs`、`/openapi.json`、`/redoc` 等路径不走 SPA 回退
- **构建脚本**: `scripts/build_web.sh` 编译 Vue 前端
- **环境变量**: `LELAMP_WEB_DIST` 控制构建输出路径（默认 `web/dist`）

---

### 6. LiveKit Token API 🆕

`POST /api/livekit/token` 端点替代旧的 CLI 脚本 `scripts/tools/generate_client_token.py`：

```python
class LiveKitTokenRequest(BaseModel):
    room: str = Field(..., min_length=1, max_length=128)  # 房间名验证
    identity: str | None = None  # 用户身份（可选，JWT 认证时强制使用）
```

**安全特性**:
- 需要 JWT 认证
- 强制使用已认证用户的身份（`identity`），防止身份伪造
- 房间名通过 Pydantic Field 验证
...
**行为**:
- `LELAMP_JWT_SECRET` 未设置时，自动生成随机密钥并输出警告日志
- 随机密钥在进程重启后会变化，所有现有 token 失效
- 生产环境务必通过环境变量设置固定密钥

---

### 8. 数据库架构 (`lelamp/database/`)
...
AP 配网热点每次启动时自动生成随机密码（替代固定密码 "lelamp123"）：

```python
# APConfig.password 默认值从 "lelamp123" 改为 None（自动生成）
password = secrets.token_urlsafe(6)  # 8 字符 URL 安全随机密码
```

**关键行为**:
- 密码通过 `secrets.token_urlsafe(6)` 生成，每次 AP 启动时不同
- 密码持久化到 `setup_status.json`，Captive Portal 欢迎页显示密码提示
- AP 模式下 LED 呈蓝色呼吸效果
- 旧固定密码已移除，提升安全性
